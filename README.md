# Code Skills - Claude 代码助手技能集合

> Claude Code 技能插件集合，包含 code-partner、code-documenter 和 code-interview 三个独立技能。

> ⚠️ **注意**：本项目仅在 Windows 11 下测试通过。

## 📦 技能列表

### 1. code-partner - 资深编程伙伴

让 Claude 成为项目的"守门员"，在重要代码改动前先做技术判断，而不是直接动手修改。

- **技术益害评估**：在重要代码变更前先进行益处检查和风险评估
- **风险预警机制**：对潜在隐患发出警告并提供替代方案
- **分层响应**：默认先给轻量评估，必要时再输出完整优化日志
- **优化日志系统**：对重要代码变更使用 Markdown 记录目的、内容和验证标准
- **触发边界清晰**：优先用于先分析、评估、权衡再实施的开发任务，避免误用于只读和轻量任务

### 2. code-documenter - 项目文档生成器

自动分析项目代码和文档，生成结构化的学习文档。

- **项目分析**：自动扫描代码结构，识别模块依赖关系
- **文档生成**：生成包含架构图、流程图、模块详解的学习文档
- **可视化图表**：使用 Mermaid 生成清晰的流程图和架构图

### 3. code-interview - 代码面试文档生成器

系统性地梳理项目发展历史，输出专业文档用于面试准备和技术债梳理。

- **Git记录分析**：提取重大变更提交，标注时间/模块/变更类型
- **文档挖掘**：从项目文档中提取架构调整和技术选型决策
- **代码实现分析**：分析代码结构还原实现层面的重大变更
- **双文档输出**：
  - 完整变更历史（背景/原因/代码细节/效果/风险/优化方向）
  - 重难点专题（跨模块架构、高技术难点、历史遗留问题）

## 📁 项目结构

```
codePartner/
├── skills/
│   ├── code-partner/            # 资深编程伙伴技能
│   │   ├── SKILL.md
│   │   └── references/
│   │       ├── RISK_ASSESSMENT.md
│   │       └── LOG_TEMPLATE.md
│   │
│   ├── code-documenter/         # 文档生成器技能
│   │   ├── SKILL.md
│   │   ├── references/
│   │   │   ├── DOCUMENT_TEMPLATE.md
│   │   │   └── DIAGRAM_STYLES.md
│   │   └── scripts/
│   │       ├── analyze_project.py
│   │       └── generate_docs.py
│   │
│   └── code-interview/          # 代码面试文档生成器技能
│       └── SKILL.md
│
├── examples/                    # code-partner 使用示例
├── docs/                        # 详细文档
└── README.md
```

## 🚀 快速开始

### 通过 Claude Plugin Marketplace 安装（推荐）

```bash
# 1. 添加 Marketplace 源
/plugin marketplace add Paranoid49/skills

# 2. 安装技能（可单独安装或全部安装）
/plugin install code-partner      # 资深编程伙伴
/plugin install code-documenter   # 文档生成器
/plugin install code-interview    # 代码面试文档生成器
```

## 📚 详细文档

### code-partner

- [完整使用指南](docs/code-partner/usage.md)
- [风险评估详解](docs/code-partner/risk_assessment.md)
- [日志格式规范](docs/code-partner/log_format.md)
- [使用示例](examples/code-partner)

### code-documenter

- [技能说明](skills/code-documenter/SKILL.md)
- [文档模板](skills/code-documenter/references/DOCUMENT_TEMPLATE.md)
- [图表样式指南](skills/code-documenter/references/DIAGRAM_STYLES.md)
- [使用示例](examples/code-documenter)

### code-interview

- [技能说明](skills/code-interview/SKILL.md)

## 🎯 code-partner 输出模式

- **轻量评估模式（默认）**：先给出益处、风险和决策，适合大多数开发任务的前置判断
- **完整日志模式（按需使用）**：在重要代码变更完成后，再输出完整日志条目并写入 `project_logs/optimization/YYYY-MM-DD.md`

## 🎓 code-partner 工作流程

```
任务请求 → 技术益害评估 → 风险判断
                                ↓
                    ┌───────────┴───────────┐
                    ↓                       ↓
               直接执行              警告 + 替代方案
                    ↓                       ↓
        重要代码变更时记录日志 ←──────────────┘
                    ↓
              补充必要测试
                    ↓
              核心功能验证
                    ↓
                 ✅ 完成
```

## 📋 日志示例

以下示例属于**完整日志模式**，通常在重要代码变更完成后使用：

```markdown
### 【优化日志条目】

**变更类型**：优化
**时间戳**：2026-02-07 14:30:00
**负责人**：Claude

#### 变更目的
优化用户查询接口性能，解决 N+1 问题

#### 变更内容
- **具体修改**：使用 SQLAlchemy 的 `selectinload()` 预加载
- **涉及模块**：`user_service`, `database`
- **实施方法**：批量预加载关联数据

#### 验证标准
- **成功指标**：API 响应时间 < 150ms
- **测试方案**：补充关键路径测试 + 性能基准测试
- **风险评估**：需监控内存占用

#### 后续建议
- **相关影响**：其他列表查询可参考此方案
- **待办事项**：审查其他循环查询代码
```

## 🛡️ 风险评估指南

code-partner 内置完整的风险评估参考，覆盖：

- 🏗️ 架构风险（单例模式、循环依赖、紧耦合）
- ⚡ 并发风险（竞态条件、死锁、资源泄漏）
- 📊 性能风险（API成本、内存爆炸、延迟增加）
- 🔧 可维护性风险（技术债务、测试盲区）
- 🔒 安全风险（注入攻击、敏感数据）

详见 [docs/code-partner/risk_assessment.md](docs/code-partner/risk_assessment.md)

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📄 许可证

MIT License
